<template>
            ...
            <div class="col-md-8">
                ...
                <card-component titulo="Relação de Locações">
                    <template v-slot:conteudo>
                        <table-component :dados="locacoes.data" :visualizar="{
                            visivel: true,
                            dataToggle: 'modal',
    ...
    status: { titulo: 'Status da Locação', tipo: 'status' }
                        ...
                        <div class="form-group">
                            <input-container-component titulo="Placa do Carro" id="placaCarro" id-help="placaCarroHelp"
                                ...
                                <select class="form-select" v-model="placaCarro">
                                    <option v-for="item, key in carros.data" :key="key" :value="item[0]">{{ item[1] }}
                                    </option>
                                </select>
                            ...
                            <input-container-component titulo="Nome do Cliente" id="nomeCliente" id-help="nomeClienteHelp"
                                ...
                                <select class="form-select" v-model="nomeCliente">
                                    <option v-for="item, key in clientes.data" :key="key" :value="item[0]">{{ item[1] }}
                                    </option>
                                </select>
                            </input-container-component>
                            <input-container-component titulo="Início do Aluguel" id="inicioAluguel"
                                id-help="inicioAluguelHelp" texto-ajuda="(Obrigatório) Informe o nome do modelo">
                                <input type="date" class="form-control" id="incioAluguel"
                                    aria-describedby="incioAluguelHelp" v-model="inicioAluguel" required>
                            </input-container-component>
                            <input-container-component titulo="Data Final do Aluguel (previsto)" id="dataFinalPrevista"
                                id-help="dataFinalPrevistaHelp"
                                texto-ajuda="(Obrigatório) Informe a Data Final do Aluguel (previsto)">
                                <input type="date" class="form-control" id="dataFinalPrevista"
                                    aria-describedby="dataFinalPrevistaHelp" v-model="dataFinalPrevista" required>
                            </input-container-component>
                            ...
                            <input-container-component titulo="KM inicial do Veículo" id="kmVeiculoIncio"
                                id-help="kmVeiculoIncioHelp"
                                texto-ajuda="(Obrigatório) Informe o km do veículo antes do aluguel">
                                <input type="text" class="form-control" id="kmVeiculoIncio"
                                    aria-describedby="kmVeiculoIncioHelp" v-model="kmVeiculoIncio" required>
                            </input-container-component>
                        </div>
                    </template>
                    <template v-slot:rodape>
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                        <button type="button" class="btn btn-primary" @click="salvar()">Salvar Locação</button>
                    </template>
                </modal-component>
                ...
                <modal-component id="modalLocacoesVisualizar" titulo="Dados da Locação">
                    ...
                    <template v-slot:rodape>
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                        <button type="button" class="btn btn-danger" @click="cancelarLocacao()">Cancelar Locação</button>
                    </template>
                </modal-component>
                ...
                <modal-component id="modalLocacoesFinalizar" titulo="Finalizar Locação">
                    <template v-slot:conteudo>
                        <div class="form-group">
                            <input-container-component titulo="Data de Entrega" id="dataFinal" id-help="dataFinalHelp"
                                texto-ajuda="(Obrigatório) Informe a Data Final do Aluguel">
                                <input type="date" class="form-control" id="dataFinal" aria-describedby="dataFinalHelp"
                                    v-model="dataFinal" required>
                            </input-container-component>
                            <input-container-component titulo="KM final do veiculo" id="kmVeiculoFinal"
                                id-help="kmVeiculoFinalHelp"
                                texto-ajuda="(Obrigatório) Informe o km do veículo após a locação">
                                <input type="text" class="form-control" id="kmVeiculoFinal"
                                    aria-describedby="kmVeiculoFinalHelp" v-model="kmVeiculoFinal" required>
                            </input-container-component>
                            <input type="hidden" v-model="$store.state.item.data_inicio_periodo" id="dataInicio">
                        </div>
                    </template>
                    <template v-slot:rodape>
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                        <button type="button" class="btn btn-primary" @click="finalizarLocacao()">Finalizar Locação</button>
                    </template>
                </modal-component>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        finalizarLocacao() {
            ...
            let kmPercorrido = this.kmVeiculoFinal - this.$store.state.item.km_inicial;
